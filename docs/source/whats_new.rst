...
Bugs
~~~~
- Fix :class:`moabb.datasets.Patel2025` so a truncated 119-sample left-hand field from subject 2 is skipped instead of being zero-padded into the declared 1 s (256-sample) analysis interval, while retaining the valid right-hand and rest fields from the same MATLAB struct element. Version the raw-cache fingerprint so caches containing the synthetic padded epoch are not reused, and clarify that the redistributed amplitudes are undocumented, uncalibrated native values for which no physical rescaling is supported by the public metadata (by `Bruno Aristimunha`_).
